...
const POPULAR = [
  { title: "Click, Order, Eat: The Convenience of FDA",  date: "January, 17 2024", image: "/Mask group (12).png" },
  { title: "Efficiency and Flavor: The Promise of FDA",  date: "January, 17 2024", image: "/Mask group (13).png" },
  { title: "Reshaping the Restaurant Industry",          date: "January, 17 2024", image: "/Mask group (14).png" },
  { title: "Limit of oxygen level in your blood",        date: "January, 17 2024", image: "/Mask group (15).png" },
];
  ...
  return (
    <aside className="w-full lg:w-[433px] flex-shrink-0 flex flex-col gap-6 lg:gap-8">
        ...
        <div className="flex flex-col gap-5">
          {POPULAR.map((p, i) => (
            <div key={i} className="flex items-center gap-4">
              ...
              <div>
                <p className="font-heading font-medium text-[16px] sm:text-[18px] leading-[150%] tracking-[0.02em] text-[#313131]">{p.title}</p>
                <p className="font-heading font-medium text-[14px] sm:text-[16px] tracking-[0.02em] text-[#EF5B5B] mt-1">{p.date}</p>
                <p className="font-heading font-normal text-[14px] leading-[163%] tracking-[0.02em] text-neutral-500">Javier Bardem</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="border border-neutral-300 rounded-[30px] p-6 lg:p-8">
        ...
        <p className="font-heading font-normal text-[18px] sm:text-[20px] leading-[150%] tracking-[0.02em] text-neutral-600 mt-3">
          Don&apos;t miss a thing! Sign up to recieve daily deals
        </p>
        <input type="email" placeholder="Enter your Email" className="mt-5 w-full bg-neutral-100 border border-[#e1e1e1] rounded-[50px] h-[56px] px-6 outline-none font-heading font-normal text-[16px] text-neutral-800 placeholder:text-neutral-500" />
      ...
      <div className="border border-neutral-300 rounded-[30px] p-6 lg:p-8">
        ...
        <div className="flex flex-col gap-4">
          {CATEGORIES.map((c) => (
            <div key={c.name} className="flex items-center justify-between border-b border-neutral-100 pb-4 last:border-0 last:pb-0">
              <span className="font-heading font-medium text-[18px] sm:text-[20px] tracking-[0.02em] text-neutral-800">{c.name}</span>
              <span className="font-heading font-normal text-[18px] sm:text-[20px] tracking-[0.02em] text-neutral-500">({c.count})</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          {TAGS.map((tag, i) => (
            <button
              key={tag}
              className={cn(
                "rounded-[50px] h-[40px] px-5 font-heading font-medium text-[14px] sm:text-[16px] transition-colors",
