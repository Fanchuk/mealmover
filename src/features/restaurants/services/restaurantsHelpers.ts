...
export function buildPrismaWhereClause(filters: RestaurantsFilters): Prisma.RestaurantWhereInput {
  ...
  if (filters.tags && filters.tags.length > 0) where.tags = { some: { name: { in: filters.tags } } };
  if (filters.category === "healthy") where.tags = { some: { name: { equals: "Healthy", mode: "insensitive" } } };
...
export function sortAndFilterRestaurants(items: RestaurantListItem[], filters: RestaurantsFilters) {
  let processed = [...items];
  ...
  if (withDistance && (filters.category === "near" || filters.sort === "distance")) {
    ...
    processed.sort((a, b) => b.favoritesCount - a.favoritesCount);
