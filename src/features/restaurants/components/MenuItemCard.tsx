...
export function MenuItemCard({ id, name, desc, price, oldPrice, discount, image, restaurantId, restaurantName, onOpenModal }: Props) {
  ...
  return (
    <motion.div
      ...
      className="group border border-neutral-300 rounded-[30px] sm:rounded-[40px] p-4 flex gap-4 bg-white hover:border-[#EF5B5B] hover:shadow-[0_16px_40px_-16px_rgba(239,91,91,0.3)] transition-colors"
    >
      <button onClick={onOpenModal} className="overflow-hidden rounded-[20px] flex-shrink-0 cursor-pointer" aria-label={`Customize ${name}`}>
        <img ref={imageRef} src={image} alt={name} className="w-[120px] sm:w-[180px] h-[120px] sm:h-[180px] object-cover transition-transform duration-500 group-hover:scale-110" />
      </button>
      <div className="flex flex-col gap-1.5 sm:gap-2 min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <span className="font-heading font-semibold text-[20px] sm:text-[31px] text-[#EF5B5B]">${price.toFixed(2)}</span>
