...
export function MenuItemCard({ id, name, desc, price, oldPrice, discount, image }: Props) {
  const imageRef = useRef<HTMLImageElement>(null);

  return (
    <div className="border border-neutral-300 rounded-[30px] sm:rounded-[40px] p-4 flex gap-4 bg-white">
      <img ref={imageRef} src={image} alt={name} className="w-[120px] sm:w-[180px] h-[120px] sm:h-[180px] object-cover rounded-[20px] flex-shrink-0" />
      <div className="flex flex-col gap-1.5 sm:gap-2 min-w-0 flex-1">
        ...
        <AddToCartStepper id={id} name={name} price={price} image={image} imageRef={imageRef} />
      </div>
    </div>
  );
}