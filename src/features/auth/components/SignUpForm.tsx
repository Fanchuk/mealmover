...
import { registerUser, type AuthState } from '../../checkout/services/authActions'
import { AuthLayout } from "./AuthLayout";

export function SignUpForm() {
  const router = useRouter();
  const [state, formAction, pending] = useActionState(registerUser, { ok: false } as AuthState);
  ...
  return (
    <AuthLayout>
      <h1 className="font-heading font-bold text-[34px] text-neutral-800">Create account</h1>
      <p className="font-heading text-[15px] text-neutral-500 mt-2 mb-8">
        Already have an account?{" "}
        <Link href="/sign-in" className="text-[#EF5B5B] font-medium hover:underline">Sign in</Link>
      </p>

      <button
        onClick={() => signIn("google", { callbackUrl: "/restaurants" })}
      ...
      <form action={formAction} className="flex flex-col gap-4">
        <Field icon={<User size={18} />} name="name" type="text" placeholder="Full name" />
